...
import (
	...
	"meta_service/common/httputil/utils"
...
func SubmitJobHandleReq(ctx *gin.Context) *SubmitRequest {
	traceID := ctx.Request.Header.Get(constants.HeaderTraceID)
	...
	req.EntrypointMemory = int(math.Ceil(float64(req.EntrypointMemory) / constants.MemoryUnitConvert / constants.MemoryUnitConvert))
