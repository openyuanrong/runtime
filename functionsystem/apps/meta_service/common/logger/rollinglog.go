...
func initRollingLog(coreInfo config.CoreInfo, flag int, perm os.FileMode) (*rollingLog, error) {
	...
	if coreInfo.Rolling == nil {
		coreInfo.Rolling = &config.RollingInfo{
			MaxSize:    defaultMaxSize,
			MaxBackups: defaultMaxBacks,
		}
	}
	log := &rollingLog{
		...
		flag:         flag,
		perm:         perm,
		maxSize:      int64(coreInfo.Rolling.MaxSize * megabyte),
		maxBackups:   coreInfo.Rolling.MaxBackups,
		isUserLog:    false,
	}
	if log.maxBackups < 1 {
