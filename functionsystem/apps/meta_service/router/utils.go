 ...
 */
// Package router define utils tools  for meta service.
package router
...
func shouldBindJSON(c server.Context, v interface{}) error {
	...
	if err := decoder.Decode(v); err != nil {
		...
		return snerror.New(errmsg.InvalidParamErrorCode, err.Error())
	}
	return utils.Validate(v)
}

// successJSONBodyForUser structure returned when the processing is successful.
type successJSONBodyForUser struct {
	Code     int         `json:"code"`
