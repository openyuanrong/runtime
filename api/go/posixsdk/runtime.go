...
func InitRuntime(conf *common.Configuration, intfs execution.FunctionExecutionIntfs) error {
	...
	initArgsFilePath := os.Getenv(initArgsFilePathEnvKey)
	if conf.DriverMode && len(initArgsFilePath) != 0 {
		initArgsData, err := os.ReadFile(initArgsFilePath)
		if err != nil {
			fmt.Printf("failed to read init args file %s, error %s\n", initArgsFilePath, err.Error())
			return err
		}
		return bootstrapSystemFunction(intfs, initArgsData)
	}
	return nil
}

func bootstrapSystemFunction(intfs execution.FunctionExecutionIntfs, initArgsData []byte) error {
	...
	if err := intfs.FunctionExecute(funcMeta, config.CreateInstance, args, []config.DataObject{}); err != nil {
		fmt.Printf("failed to call init for function %s, error %s\n", funcName, err.Error())
		return err
	}
	return nil
}
...
func ExecShutdownHandler(signum int) {
	libruntime.ExecShutdownHandler(signum)
}
