...
When multiple businesses or environments such as development, testing, etc. share a set of K8s, you can create multiple custom Pod resource pools and label them according to business or environment type, use openYuanrong's [affinity scheduling strategy](../../../multi_language_function_programming_interface/development_guide/scheduling/affinity.md) to assign function instances to specific Pods to achieve isolation. In addition, [Create Pod Resource Pool API](../api/create_pod_pool.md) provides `node_selector` and `affinities` fields, natively supports [k8s pod affinity scheduling](https://kubernetes.io/zh-cn/docs/concepts/scheduling-eviction/assign-pod-node/){target="_blank"}, to assign created Pods to specific nodes. Combining the two can achieve more flexible resource matching.
