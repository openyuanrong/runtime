...
主节点 pod 用于管理集群，负责全局函数调度、请求转发等工作。包含的组件有 function master、FaaS manager、FaaS scheduler、fronted、meta service、IAM adaptor 及开源 MinIO、etcd。
...
- **FaaS manager**
  ...
  负责租约申请与回收，清理过期连接信息。它是 openYuanrong 系统函数。

- **FaaS scheduler**

  负责函数服务的调度。它是 openYuanrong 系统函数。

- **frontend**

  提供 REST API 用于调用服务、订阅流服务等数据处理。它是 openYuanrong 系统函数。
