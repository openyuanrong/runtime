...
- Driver 程序中调用 `YR::Init(const Config &conf)` 接口时，配置 [Config](../../multi_language_function_programming_interface/api/distributed_programming/Cpp/struct-Config.rst) 的 `loadPaths` 参数。
...
- 应用程序中调用 `init(Config conf)` 接口时，配置 [Config](../../multi_language_function_programming_interface/api/distributed_programming/Java/Config.md) 的 `loadPaths` 参数。
