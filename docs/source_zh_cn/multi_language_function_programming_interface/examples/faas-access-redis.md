...
2. 安装函数服务 [Java SDK](install-yuanrong-java-sdk) 中的 faas-function-sdk 与 [MinIO Client](tools-minio-client)。MinIO Client 用于上传代码包到 openYuanrong 集群中的 MinIO 服务。
3. 已部署 Redis 集群。
...
结果返回格式如下，记录 `functionVersionUrn` 字段的值用于调用，这里对应 `sn:cn:yrk:default:function:0@faas@access-redis:latest`
